...
"q=p+vec3(-3.25,2.,3.);"
"r=m(r,vec2(length(max(abs(q)-vec3(.2,.2,4.),0.)), MAT_OHP)); "
"r=m(r,vec2(length(max(abs(q+vec3(.1,-.4,2.5))-vec3(.5,.8,.5),0.)), MAT_OHP)); "
"q+=vec3(1.4,-.8,3.1);"
"q.zx *= rot2(1.);"
"q.zy *= rot2(-.3);"
"r=m(r,vec2(length(max(abs(q)-vec3(.2,.2,1.7),0.)), MAT_OHP)); "
"p.z += 7.;"
"r=m(r,vec2(max(dot(p-vec3(0.,0.,.3),vec3(0.,0.,-1.)),length(p)-.6),MAT_BLU)); "
"r=m(r,vec2(max("
"length(max(abs(p)-vec3(.7,1.,.3),0.)), "
"dot(p,normalize(vec3(0.,1.,-2.))) "
"), MAT_OHP));"
"p.z+=.8;"
"p.yz*=rot2(-.9);"
...
"p.y-=35.;"
"p.z+=28.; "
"float t = dot(p+vec3(0.,0.,2.3),-vec3(0.,.43,1.)); "
"t=max(t,dot(p+vec3(8.1,0.,0.),-vec3(1.,.22,0.))); "
"t=max(t,dot(p-vec3(8.1,0.,0.),vec3(1.,-.22,0.))); "
"t=max(t,dot(p-vec3(0.,0.,13.7),vec3(0.,0.,1.))); "
"return max(t,dot(p+vec3(0.,35.,-13.2),vec3(0.,-.1,-0.08))); "
"}"
"vec2 map(vec3 p)"
"{"
"p.z += 2.;"
...
"res = min(res, 128.*d/max(0.,dist-y)); "
"ph = h;"
"dist += h;"
"}"
"return res;"
